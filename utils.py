...
def stoch_rsi(df, period=14):
    ...
    rsi = 100 - (100 / (1 + rs))
    return (rsi - rsi.rolling(period).min()) / (rsi.rolling(period).max() - rsi.rolling(period).min() + 1e-9)
...
def mfi(df, period=14):
    ...
    return 100 * pos.rolling(period).sum() / (pos.rolling(period).sum() + neg.rolling(period).sum() + 1e-9)
